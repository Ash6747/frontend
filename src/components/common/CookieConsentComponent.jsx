...
import "../../assets/css/concentForm.css";
...
const CookieConsentComponent = ({ isVisibleCookie = "mobile" }) => {
  useEffect(() => {
    config.onFirstConsent = ({ cookie }) => {
      const payload = {
        consentSelections: cookie.categories,
    ...
    const cc = CookieConsent.run(config);

    cc.then(() => {
      const closeButton = document.getElementById("cc-footer-close");
      if (closeButton) {
      ...
      const necessaryBtn = document.querySelector('.cm__btn[data-role="necessary"]');
      const preferencesBtn = document.querySelector('.cm__btn[data-role="show"]');

      if (necessaryBtn && preferencesBtn) {
        preferencesBtn.insertAdjacentElement("afterend", necessaryBtn);
      }
    });
  ...
  // return (
  //   <div className={`cookie_cta ${isVisibleCookie}`}>
  //     <button type="button" onClick={CookieConsent.showPreferences} aria-label="cookie-btn">
